...
pub fn BoardComponent(
    ...
    let get_pos = |mut depot: usize, mut ord: usize| {
        loop {
            let (role, index) = DepotRole::role_and_subindex(depot).unwrap();
            match role {
                DepotRole::FreeCell => 
                    return Vec2::new(center_x(4, index), pos_y(0)),
                DepotRole::Tableau => 
                    return Vec2::new(center_x(5, 0) + row_card_offset.x * ord as f32, pos_y(index + 1)),
                DepotRole::Shadow => {
                    depot = index; ord = 0;
                }
            }
    ...
    let get_hint = |depot: usize| {
        let (role, index) = DepotRole::role_and_subindex(depot).unwrap();
        match role {
            DepotRole::FreeCell => Some(
                    rsx! {
                        if index < num_freecells_unlocked {
                            span {
